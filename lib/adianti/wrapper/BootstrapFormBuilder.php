...
class BootstrapFormBuilder implements AdiantiFormInterface
{
    ...
    public function render()
    {
        ...
        if ($this->tabcurrent !== null)
        {
            ...
            {
                $class = ($tab_counter == $this->current_page) ? 'active' : ''; //isso aqui tá verificando se a aba é a atual.
                ...
                /*
                if(str_contains($tab_link->{'class'},"active")){ //aí verifca se é a aba atual.
                    //estilos que tu quiser
                   //$tab_link->{'style'} .= 'border: 7px solid var(--bs-border-color)';
                   $tab_link->{'style'} .= 'font-size: 20px'; 
                }*/
        ...
        foreach ($this->tabcontent as $tab => $rows)
        {
            ...
            $tabpanel->{'class'} = 'tab-pane tabpanel_'.$this->name .' '. ( ($tab_counter == $this->current_page) ? 'active' : '' ); //ternário ? :
            $tabpanel->{'style'} = 'padding:10px; margin-top: -1px;';
            if ($tab)
            {
                $tabpanel->{'style'} .= 'border: 2px solid var(--bs-border-color)';
            }
