    ...
    public static function onLogin($param)
    {
        ...
        try
        {
            ...
            if ($user)
            {
                ...
                // 🔹 PEGAR OS GRUPOS DO USUÁRIO
                TTransaction::open('permission');
                $groups = $user->getSystemUserGroups();
                $group_names = [];
                ...
                // Salvar os grupos do usuário na sessão
                TSession::setValue('user_groups', $group_names);

                // Se quiser só o primeiro grupo:
                if (!empty($group_names)) {
                    TSession::setValue('user_group', $group_names[0]);
                }
                TTransaction::close();
                // 🔹 FIM DO TRECHO DE GRUPOS
                ...
                {
                    $grupos = TSession::getValue('user_groups'); // retorna array com todos os grupos
                    //$grupo  = TSession::getValue('user_group');  // retorna só o primeiro
                    
                    // 🔹 AQUI ENTRA A TUA REGRA DE GRUPO
                    if (in_array('Funcionários', $group_names)) {
                        AdiantiCoreApplication::gotoPage('notebookteste');
                        TSession::setValue('frontpage', 'notebookteste');
                    } else {
