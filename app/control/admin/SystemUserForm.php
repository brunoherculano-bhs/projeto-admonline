...
class SystemUserForm extends TPage
{
    ...
    public function __construct()
    {
        ...
        //$id->setVisibility(false);

        $xdatlim->setMask('dd/mm/yyyy');
        ...
        $this->form->addFields( [new TLabel('Data Limite')], [$xdatlim] );
    ...
    public function onSave($param)
    {
        ...
        try
        {
            ...
            $object->datlim     = TDate::date2us($object->datlim);
            ...
            if( !empty($data->role_list) )
            {
                foreach( $data->role_list as $role_id )
                {
                    $object->addSystemUserRole( new SystemRole($role_id) );
                       ...
                       // envio de email ao final

            $username = TSession::getValue('userunitname');
            $emptel = TSession::getValue('userunittel');
            $empema = TSession::getValue('userunitema');
            //new TMessage('info',$username);
            ...
            try {
                ...
                // aqui você coloca o template de e-mail
                $html = '
                <!DOCTYPE html>
                <html lang="pt-BR">
                <head>
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    ...
                    <td align="center" style="padding:20px 10px;">
                        <table border="0" cellpadding="0" cellspacing="0" width="600" style="background:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 6px rgba(0,0,0,0.1);">
                        
                        <tr>
                            <td align="center" bgcolor="#4a90e2" style="padding:30px; color:#ffffff; font-size:24px; font-weight:bold;">'."📩 Bem vindo a ". $username .'</td>
                        </tr>

                        <tr>
                            <td style="padding:30px; color:#333333; font-size:16px; line-height:1.5;">
                            <p>Olá <strong>' . $data->name . '</strong>,</p>
                            <p>Estamos muito felizes em tê-lo conosco e daremos início ao processo de pré-admissão online.</p>
                            </br>
                            <p>Por favor, acesse o link abaixo e preencha suas informações cadastrais até o dia <strong>'. $data->datlim . '</strong> (ou dentro do prazo informado pela empresa):</p>                           

                            <table width="100%" cellpadding="10" cellspacing="0" style="margin:20px 0; border:1px solid #e0e0e0; border-radius:6px;">
                                <tr style="background:#f9f9f9;">
                                <td style="font-weight:bold; width:30%;">Link de acesso:</td>
                                <td><a href="https://www.sistemasgeo.com.br/admonline">Clique aqui</a></td>
                                </tr>
                                <tr>
                            ...
                            <p>⚠️ O preenchimento dentro do prazo é essencial para que o processo de admissão ocorra sem atrasos.</p>
                            </br>
                            <p>Após o envio das informações, nossa equipe entrará em contato para dar continuidade ao processo de admissão.</p>  
                            </br>
                            ...
                            <!--<p style="margin-top:20px;">Recomendamos trocar a senha no primeiro acesso.</p>-->
                            </td>
                        </tr>

                        <tr>
                            <td align="center" bgcolor="#f0f0f0" style="padding:20px; font-size:12px; color:#666;">
                            &copy; 2025 Micromoney — Todos os direitos reservados.
                            </td>
                        </tr>
                ...
                // envia o email como HTML
                MailService::send(
                    trim($data->email),
                    'ENCAMINHAMENTO DE ADMISSÃO - ACESSO',
                    $html,
                    'html'
                );
